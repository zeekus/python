...
class Portfolio:
    ...
    def add_asset_purchase(self, asset_name, quantity, usd_amount, fee):
        net_quantity = quantity  # Corrected: do not account for fee in quantity
    ...
    def add_transaction(self, asset_name, quantity, amount_usd, fee, time):
        ...
        if amount_usd < 0:
            ...
            net_quantity = quantity  # Corrected: no need to subtract fee here
            purchase_price_per_unit = -amount_usd / net_quantity
            self.add_asset_purchase(asset_name, net_quantity, -amount_usd, fee)
        else:
            # Sale
