...
def run_bot(starting_price):
    pair = 'SUIUSD'  # Kraken uses this format for SUI/USD
    check_interval = 60  # Check every minute
    intervals = [1, 5, 30, 60, 240]  # 1m, 5m, 30m, 1h, 4h
    last_update = {interval: datetime.now() - timedelta(minutes=interval) for interval in intervals}
    data_cache = {interval: pd.DataFrame() for interval in intervals}
    ...
    def trading_strategy(pair, starting_price):
        ...
        while True:
            ...
            for interval in intervals:
                if first_run or (current_time - last_update[interval]).total_seconds() >= interval * 60:
                    data_cache[interval] = get_ohlc_data(pair, interval)
                    last_update[interval] = current_time
                    analyze_data(data_cache[interval], interval)

            first_run = False

            current_price = float(data_cache[1]['close'].iloc[-1])
