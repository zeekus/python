...
def calculate_profits(df):
            ...
            if amount > 0:  # Buying asset
                ...
                while sell_amount > 0 and open_positions[asset]:
                    ...
                    if buy['amount'] <= sell_amount:
                        ...
                        buy['cost'] *= (buy['amount'] / (buy['amount'] + sell_amount))
                        sell_amount = 0
    ...
    return pd.DataFrame(profits), asset_profits, total_profit
...
#         if row['type'] == 'trade':
#             if row['asset'] == 'USD':
#                 if row['amount'] < 0:  # Buying non-USD asset
#                     open_positions[row['refid']] = {'amount': abs(row['amount']), 'asset': ''}
#                 else:  # Selling non-USD asset
#                     buy_info = open_positions.pop(row['refid'], {'amount': 0, 'asset': ''})
#                     profit = row['amount'] - buy_info['amount'] - row['fee']
#                     asset = buy_info['asset']
#                     profits.append({
#                         'time': row['time'],
#                         'refid': row['refid'],
#                         'asset': asset,
...
#             else:
#                 if row['amount'] > 0:  # Buying non-USD asset
#                     open_positions[row['refid']]['asset'] = row['asset']
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Trade Profit Calculator', add_help=False)
    ...
    profits_df, asset_profits, total_profit = calculate_profits(df)
