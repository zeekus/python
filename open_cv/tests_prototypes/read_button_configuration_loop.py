#!/usr/bin/python3
#filename: read_button_configuration_loop.py
#descrption: program uses open computer vision to determine what the buttons on the overview are configured as
#this is a prototype 
...
def  loop_through_messages(file):
  ...
  for i in data:
    ...
    path=os.getcwd()
    full_path_file=(path + "/buttons/" + filename)
...
path=os.getcwd()
message_json=(path + "/buttons/buttons.json")
