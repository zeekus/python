...
class Trade:
    def __init__(self, date, quantity, price, asset, cash_balance):
        self.date = date
    ...
    def print_trade(self):
        print(f'Date: {self.date}, Asset: {self.asset}, Quantity: {self.quantity:.8f}, Price: ${self.price:.4f}, Cash Balance: ${self.cash_balance:.4f}')
...
for _, row in df.iterrows():
    if row['type'] == 'trade':
        ...
        if quantity != 0:  # Skip rows with zero quantity
            ...
            trade = Trade(date, quantity, price, asset, cash_balance)
