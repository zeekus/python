...
class Trade:
    ...
    def print_trade(self, line_number):
        ...
        print(f'   Pair: {self.quote_asset}/{self.base_asset}')
        print(f'   Action: {action}')
        print(f'   {self.quote_asset}: {abs(self.quote_quantity):.8f}')
        print(f'   {self.base_asset}: {abs(self.base_quantity):.8f}')
        print(f'   Price: {self.price:.8f} {self.quote_asset}')
        print(f'   Fee: ${self.fee:.6f}')
        if self.is_buy:
            print(f'   Buying {abs(self.base_quantity)} {self.base_asset} at {self.price:.8f} {self.quote_asset} each for a total of {abs(self.quote_quantity):.8f} {self.quote_asset}')
        else:
            print(f'   Selling {abs(self.base_quantity)} {self.base_asset} at {self.price:.8f} {self.quote_asset} each for a total of {abs(self.quote_quantity):.8f} {self.quote_asset}')
    ...
    def sell(self, trade):
        ...
        while sell_quantity > 0 and asset_queue:
            ...
            if oldest_trade.base_quantity <= sell_quantity:
                ...
                print(f'   Sold {oldest_trade.base_quantity} {trade.base_asset} bought at {oldest_trade.price:.8f}. Profit: {profit:.8f} {trade.quote_asset}')
                asset_queue.popleft()
            else:
                profit = (trade.price - oldest_trade.price) * sell_quantity
                self.pnl[trade.base_asset] += profit
                print(f'   Sold {sell_quantity} {trade.base_asset} bought at {oldest_trade.price:.8f}. Profit: {profit:.8f} {trade.quote_asset}')
