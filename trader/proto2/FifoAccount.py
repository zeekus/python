...
class FifoAccount:
    ...
    def query_fiat_wallet(self, refid):
        return self.fiat_wallet.get(refid, 0.0)

    def buy(self, trade):
        fiat_amount = self.query_fiat_wallet(trade.refid)
        trade.set_fiat_equivalent(fiat_amount)
        cost_basis = abs(fiat_amount) / trade.actual_crypto_amount if trade.actual_crypto_amount != 0 else 0.0
        
        potential_cash_balance = self.cash_balance - abs(fiat_amount)
        if potential_cash_balance < 0:
            print(f"WARNING: Insufficient funds to buy {trade.crypto_asset}. Current Cash Balance: ${self.cash_balance:.2f}, Attempted Buy Amount: ${abs(fiat_amount):.2f}")
            return
        
        self.positions[trade.crypto_asset].append((trade.actual_crypto_amount, cost_basis))
        self.cash_balance = potential_cash_balance
    ...
    def sell(self, trade):
        ...
        while sell_quantity > 0 and asset_queue:
            ...
            if oldest_trade[0] <= sell_quantity:
                ...
                profit = (abs(fiat_amount) / trade.crypto_amount - buy_price) * oldest_trade[0]
                total_profit += profit
                asset_queue.popleft()
            else:
                buy_price = oldest_trade[1]
                profit = (abs(fiat_amount) / trade.crypto_amount - buy_price) * sell_quantity
                total_profit += profit
        ...
        self.cash_balance += abs(fiat_amount)
